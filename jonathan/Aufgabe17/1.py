...
def get_x_velocity_constraints(target_area: TargetArea):
    ...
    if target_area.min_x > 0:
        ...
        min_x_velocity = get_min_x_velocity(target_area.min_x)
    elif target_area.max_x < 0:
        min_x_velocity = target_area.min_x
        max_x_velocity = -get_min_x_velocity(-target_area.max_x)
...
def get_y_velocity_constraints(target_area: TargetArea):
    ...
    max_y_velocity = get_max_y_velocity(target_area)
...
def main():
    ...
    for x_velocity in range(min_x_velocity, max_x_velocity):
        ...
        for y_velocity in range(min_y_velocity, max_y_velocity):
            curr_highest_y = hits_target_area_after_any_step(target_area, x_velocity, y_velocity)
            if curr_highest_y > highest_y:
                highest_y = curr_highest_y
    print('Highest y position:', highest_y)
